...
export const gameMiddleware = createMiddleware({
	[actions.START_GAME_REQUEST]: ({store, next, action}) => {
		...
		const cardListByPlayer: {
			[name: string]: CardState[];
		} = playerList.reduce((result, {name, initialCardNumber}) => {
			const playerCardList = stockCards.splice(0, initialCardNumber).map((value) => ({value, isVisible: false}));
